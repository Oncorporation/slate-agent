...
| Picture pairing | Video only. `none` / `imagine-ffmpeg` / `blitzreels` / `wangnov` / `cine` / `hyperframes` plus deliverable, aspect, duration budget |
| Quality band | Video default 10 until story + human lock. Then 65 or 90 |

Skip a gate only if the writer said to skip it. Export type before pairing. If type is `text`, pairing is `none` and skip Camera / Boards picture work. If `audio`, skip picture pairing. Picture pairing is last only when type is `video`.
...
8. **Camera** → `shots.md` for the scene just locked (skip if export type is `text`; optional for `audio`)
9. **Boards** → `storyboards/<shot-id>.md` with **exactly 6 frames** (video; skip `text`)
10. **Export** → typed cut under `exports/{text|audio|video}/` (video still uses `exports/{pairing}/`)
11. **Critics** → review the previous cut. Notes only. Human hold.

Producers are desks 2–11. Critics is desk 12.
...
Never have two desks overwrite the same file blindly. Architect owns `beats.md`. Scene owns Fountain. Dialogue may edit dialogue blocks only. Boards own storyboard files. Camera owns `shots.md` rows. Export owns the cut folder. Critics own `critics/` only. Never edit Fountain or `beats.md` from Export or Critics.
...
| Export | `agents/export.md` | `skills/slate-export/SKILL.md` | typed cut |
| Critics | `agents/critics.md` | `skills/slate-critics/SKILL.md` | `critics/` |
| Dialogue scout | `agents/critic-dialogue.md` | same | `critics/{cut-id}/dialogue.md` |
| Direction scout | `agents/critic-direction.md` | same | `critics/{cut-id}/direction.md` |
| Continuity scout | `agents/critic-continuity.md` | same | `critics/{cut-id}/continuity.md` |
| Scene scout | `agents/critic-scene.md` | same | `critics/{cut-id}/scene.md` |
| Picture QA | `agents/critic-picture.md` | same | `critics/{cut-id}/picture.md` |
| Audio QA | `agents/critic-audio.md` | same | `critics/{cut-id}/audio.md` |
...
- Critics never address the writer. Showrunner folds notes and holds the cut.
- Do not bump video band or land a cut without the writer's word.
